function Invoke-FileHunter {
    param(
        ...
        [string[]]$Keywords = @("password", "pass", "pwd", "credential", "secret", "key", "token"),
        ...
        try {
            ...
            ForEach-Object {
                ...
                foreach ($kw in $Keywords) {
                    ...
                    if ($matches) {
                        $results += "[$($matches.Count) matches] $file -> contains keyword '$kw'"
                    }
                }
            }
        }
    ...
    if ($results.Count -gt 0) {
        ...
        Write-Host "[+] Scan complete. Found $($results.Count) files. Results saved to $OutputLog" -ForegroundColor Green
    } else {
